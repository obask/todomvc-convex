...
The app uses a small local email/password auth flow backed by the existing Better Auth-compatible Postgres `user` and `account` tables. Passwords are stored in `account.password` as Node `scrypt` hashes, and the browser receives an HTTP-only `simple_auth_session` cookie containing a signed user payload with an expiry timestamp.
