# Solid 2 + Convex Adapter Notes

This app uses the external [`convex-solidjs`](https://github.com/obask/convex-solidjs) package instead of a local adapter file.
The package is pinned from `github:obask/convex-solidjs#dist` in [package.json](../package.json).

Reference sources:

- `node_modules/convex-solidjs/README.md`
- `node_modules/solid-js/CHEATSHEET.md`
- `node_modules/convex/dist/esm-types/` (browser + react)

## Solid 2 idioms that shape the adapter
...
- **Props are values, not accessors.** Adapter consumers pass `() => args` to `createQuery` (an explicit Accessor), but children of `Authenticated`/`Unauthenticated` are passed as raw `JSX.Element` — same as `<Show>`.
